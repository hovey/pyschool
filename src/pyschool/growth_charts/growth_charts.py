...
class DistributionPlotter:
    ...
    def __init__(self, file):
        ...
        self.months_to_years = 1.0 / 12.0  # convert months to years, dataset is in months, want plot in years
        self.data_x = None
        self.data_y = None
    ...
    def plot(self, label_x='x', label_y='y', t='title', show_plot=1, save_to_file=0, **kwargs):
        ...
        y_lim = kwargs.get('y_limits', [np.min(self.data_y[0, p05]), np.min(self.data_y[-1, p95])])
        #
        ...
        ax.fill_between(self.data_x, self.data_y[:, p05], self.data_y[:, p95], facecolor=color_str)
        ax.text(self.data_x[-1], self.data_y[-1, p05], '5', va='top', ha='right', color=color_str)  # 5th percentile
        ax.text(self.data_x[-1], self.data_y[-1, p95], '95', va='bottom', ha='right',
                color=color_str)  # 95th percentile
        #
        color_str = '#0066cc'  # blue rgb(0, 51, 102) at 40 percent
        ax.fill_between(self.data_x, self.data_y[:, p10], self.data_y[:, p90], facecolor=color_str)
        ax.text(self.data_x[-1], self.data_y[-1, p10], '10', va='top', ha='right', color=color_str)  # 10th percentile
        ax.text(self.data_x[-1], self.data_y[-1, p90], '90', va='bottom', ha='right',
                color=color_str)  # 90th percentile
        #
        color_str = '#003366'  # blue rgb(0, 51, 102) at 20 percent
        ax.fill_between(self.data_x, self.data_y[:, p25], self.data_y[:, p75], facecolor=color_str)
        ax.text(self.data_x[-1], self.data_y[-1, p25], '25', va='top', ha='right', color=color_str)  # 25th percentile
        ax.text(self.data_x[-1], self.data_y[-1, p75], '75', va='bottom', ha='right',
                color=color_str)  # 75th percentile
        #
        # color_str = '#000000'  # black
        # color_str = '#ffffff'  # white
        color_str = '#ffff00'  # yellow
        ax.plot(self.data_x, self.data_y[:, p50], linewidth=2, color=color_str)  # 50th percentile
        ax.text(self.data_x[-1], self.data_y[-1, p50], '50', va='bottom', ha='right',
                color=color_str)  # 50th percentile
        #
        ax.set_title(t)
...
if __name__ == "__main__":
    ...
    population = [('statage_male.csv', title_base + ' males ' + title_age_range),
    ...
    for item in population:
        ...
        x_label = 'age (years)'
        y_label = 'stature (cm)'
        h.plot(x_label, y_label, item[title_index], show_figure, save_figure, x_limits=[1, 21], y_limits=[70, 200])
